...
import { decode, encode, VerifyKey } from './crypto'
...
// OBLIVOUS: encoded timestamp, signed with key
export const authToken = (key) => {
  const time = (new Date()).getTime().toString()
  return encode(key.seal(new Buffer(time)))
}

export const checkToken = (id, token) => {
  const key = new VerifyKey(id)
  try {
    const time = key.open(decode(token)).toString()
