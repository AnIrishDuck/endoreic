...
import { decode, encode, BoxKeyPair } from './crypto'
...
export default class Server {
  ...
  putKey (email, publicKey, tokens) {
    const queryUrl = this.url({
      pathname: '/v1/key',
      query: { ...tokens, publicKey },
    })

    return this.api.post(queryUrl)
  }
...
export const serverKey = new BoxKeyPair(
  'LbV-X7nAoyRiFq3IsCo8A2rGMCo-F85jYfb5GFU3NSA'
)
// OBLIVOUS: encoded timestamp, boxed with server => key
export const authToken = (key) => {
  const time = (new Date()).getTime().toString()
  return encode(key.encrypt(serverKey.publicKey(), new Buffer(time)))
}
...
export const checkToken = (id, token) => {
  const time = serverKey.decrypt(id, decode(token)).toString()
  const currentTime = (new Date()).getTime()
  return Math.abs(currentTime - time) < 3600 * 1000
}
