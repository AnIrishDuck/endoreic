...
const createStoreKeyring = async (server, owner, access) => {
  ...
  // - see authTokens()
  await server.putEntry(id, 'keyring', 0, keyringBuffer, authTokens(write, owner))
  return { id, read, write, owner }
}

const getStoreKeyring = async (server, id, owner, access) => {
  // OBLIVIOUS:
  // - id is an encoded public key
...
export default class User {
  ...
  async fetchKeyring(name, ix) {
    ...
    return getStoreKeyring(this.server, first.id, this.keypair, access)
  }

  async createKeyring(name, ix) {
    const { id } = this
    const owner = this.keypair
    const access = new BoxKeyPair()
    const keyring = await createStoreKeyring(this.server, owner, access)
    const json = [
      {
    ...
    // - see authTokens()
    await this.server.putEntry(id, name, ix, ciphertext, authTokens(owner, owner))
  ...
  async getKeyring (Store) {
    ...
    if (ix > 0) {
      try {
        return await this.fetchKeyring(name, ix)
      } catch (err) {
        if (err.response && err.response.status === 404) {
          return this.createKeyring(name, ix)
        } else {
          throw err
        }
      }
  ...
  async getStore (Store) {
    return Store.create(this.db, this.server, await this.getKeyring(Store))
  }
}
...
User.create = async (server, email, password) => {
  const empty = await BoxKeyPair.fromLogin(email, '')
  const next = await BoxKeyPair.fromLogin(email, password)
  const tokens = authTokens(empty, empty)

  await server.putKey(email, next.publicKey(), tokens)
}
