...
const passThroughUndefined = (f) => (v) => v === undefined ? v : f(v)

export const Date = JsonConverter({
  toJson: passThroughUndefined((v) => v.format()),
  fromJson: (v) => moment(v),
...
export const Reference = (getCollection) => JsonConverter({
  toJson: passThroughUndefined((value) => value.id),
  ...
  validate: async (json, store) => {
    ...
    return referee.then((v) => v.id === json).catch(() => false)
  }
})
...
export const Option = _.flow(assertType('Option.inner'), (inner) => ({
  ...
  fromJson:
    (json, store) => json === null ? null : inner.fromJson(json, store),
      ...
      const pairs = await Promise.all(
        _.keys(types).concat(_.keys(json)).map((key) => {
          ...
          if (type !== undefined) {
            ...
            assert(value !== undefined, `${key} not present`)
