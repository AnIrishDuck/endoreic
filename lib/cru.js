  ...
  class Create extends Action {
    ...
    static async build (store, raw) {
      const objects = await Promise.all(raw.map(
        (o) => ({ id: o.id || uuid.v4(), ...Model.type.toJson(o) })
      ))
    ...
    errors (store) {
      return Promise.all(
        this.objects.map((raw) => Model.type.validate(raw, store))
      ).then(removeBlanks)
    }
    ...
    apply (store) {
      const strings = this.objects.map(Model.type.toSql)
  ...
  class Update extends Action {
    ...
    static async getPriors (store, { ids, update }) {
      const before = await Promise.all(ids.map((id) => (
        store.table(Model).get(id).then(Model.type.fromSql)
      )))
    ...
    static async build (store, { ids, update }) {
      ...
      const json = _.pick(Model.type.toJson(update, true), _.keys(update))
    ...
    errors (store) {
      return Promise.all(
        this.ids.map(async (id) => {
          ...
          return Model.type.validate({ ...current, ...this.update }, store)
        })
      ).then(removeBlanks)
    }
    ...
    apply (store) {
      return store.table(Model).update(this.ids, Model.type.toSql(this.update))
    }
    ...
    remove (store) {
      return Promise.all(
        _.zip(this.ids, this.priors)
          ...
          .map(([ id, prior ]) => {
            return store.table(Model).update([id], Model.type.toSql(prior))
          })
      )
    }
  }
