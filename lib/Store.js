...
export default class Store {
  ...
  store (Model) {
    ...
    return table._prepare(Model.type.keys)
  }
...
class ModelSet extends Query {
  ...
  async create (objects) {
    ...
    return ids.map((id) => this.reference(id))
  }
  ...
  _stream (statement) {
    return super._stream(statement).map((strings) => {
      const json = this.Model.type.fromSql(strings)
      return new this.Model(this.store, {
        id: json.id, ...this.Model.type.fromJson(json, this.store)
      })
    })
  }
}
